...
/**
 * REST controller for managing {@link sn.seye.gestionmatricule.mefpai.domain.Bulletin}.
 */
@RestController
@RequestMapping("/api")
public class BulletinResource {
    ...
    public ResponseEntity<Bulletin> partialUpdateBulletin(
        ...
        Optional<Bulletin> result = bulletinService.partialUpdate(bulletin);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, bulletin.getId().toString())
        );
    }
