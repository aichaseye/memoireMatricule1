...
public interface ApprenantRepository extends JpaRepository<Apprenant, Long> {
    ...
    @Query(
        value = "select distinct apprenant from Apprenant apprenant left join fetch apprenant.etablissement left join fetch apprenant.niveauEtude",
    ...
    @Query(
        "select distinct apprenant from Apprenant apprenant left join fetch apprenant.etablissement left join fetch apprenant.niveauEtude"
    )
    List<Apprenant> findAllWithToOneRelationships();

    @Query(
        "select apprenant from Apprenant apprenant left join fetch apprenant.etablissement left join fetch apprenant.niveauEtude where apprenant.id =:id"
    )
